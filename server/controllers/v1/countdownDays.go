...
type CountdownDaysController struct {
}

func getFileName(c *gin.Context, rename bool) string {
	...
	fileName := ""
	fName, err :=
		conf.SSO.AppData.Get("CountdownDaysDataFileName",
	...
	if rename {
		path := "/appData/NyaNyaToolbox/countdownDays/"
	...
	if fileName == "" || rename {
		...
		conf.SSO.AppData.Set("CountdownDaysDataFileName",
...
func (fc *CountdownDaysController) GetUploadToken(c *gin.Context) {
	// 1、请求体
	...
	if err = validation.ValidateStruct(
		...
		validation.Parameter(&data.Rename, validation.Type("bool"), validation.Required()),
	); err != nil {
		res.Errors(err)
		res.Code = 10002
		res.Call(c)
	...
	fileName := getFileName(c, data.Rename)
...
func (fc *CountdownDaysController) GetCountdownDaysFileUrls(c *gin.Context) {
	// 1、请求体
	var res response.ResponseProtobufType
	res.Code = 200

	// 2、获取参数
	data := new(protos.GetCountdownDaysFileUrls_Request)
	var err error
	if err = protos.DecodeBase64(c.GetString("data"), data); err != nil {
		res.Error = err.Error()
		res.Code = 10002
	...
	protoData := &protos.GetCountdownDaysFileUrls_Response{
		FileInfo: fi,
		Urls:     urls,
	}
